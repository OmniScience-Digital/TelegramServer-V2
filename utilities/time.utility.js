...
function calculateNormalsitesTimeIntervals(startHour, endHour) {
...
function calculateTimeIntervals(startDate, startTime, endDate, endTime) {
  ...
  function parseDateTime(dateStr, timeStr) {
    // Updated to handle the new date format "YYYY-MM-DD"
...
function getCurrentTimeShift() {
  const now = new Date();
   ...
   // Adjust to South African Standard Time (UTC+2)
   const currentHour = new Date(now.toLocaleString('en-US', { timeZone: 'Africa/Johannesburg' })).getHours();
   const prevHour = (currentHour === 0 ? 23 : currentHour - 1); // Handle midnight case
 


  const currentHourFormatted = `${currentHour.toString().padStart(2, '0')}:00`;
  const prevHourFormatted = `${prevHour.toString().padStart(2, '0')}:00`;
 

  // Determine the shift
  const shift = (currentHour > 0 && currentHour <= 23) ? "day" : "night";

  return {
    prevHour: prevHourFormatted,
    currentHour: currentHourFormatted,
    shift: shift,
  };
}
...
function addHoursToTime(time, hoursToAdd) {
  ...
  const date = new Date(); // Use a date object for easy time calculations
  date.setHours(hour, minute);
  date.setHours(date.getHours() + hoursToAdd);

  const updatedHour = date.getHours().toString().padStart(2, '0');
  const updatedMinute = date.getMinutes().toString().padStart(2, '0');
  return `${updatedHour}:${updatedMinute}`;
}

module.exports = {addHoursToTime,getPreviousDayDate,calculateTimeIntervals,getCurrentTimeShift,calculateNormalsitesTimeIntervals,
