const { plcScaleFlow, ActualPlcEnd, ActualPlcStart,ActualPlcParallelStart,ActualPlcParallelEnd, runtimeAccumulatingPlcFlow, runtimePlcFlow, averageFlowWithIccid,runtimeAccumulatingFlow,runtimeSinglePlcFlow } = require('../repositories/postgress_repository');
...
async function plcScale_noTypeCalcsFunc(monthstart, shift, startTime, endTime, startdate, enddate, plcFlow, runningtph, maxUtilization, scales, primaryScalesArray, plcIccid) {
    try {
        ...
        runtime = await runtimeSinglePlcFlow(startTime, endTime, startdate, enddate, flowiccid,plcIccid,  runningtph);
        ...
        const promises = primaryScalesArray.map(async primaryScale => {
            try {
                 ...
                 const foundScale = scales.find((scale => scale.scaleName.S === primaryScale)) || (scales.find(scale => scale.scaleName === primaryScale));
                
                if (foundScale) {
                    const iccid = foundScale.iccid.S||foundScale.iccid;
...
async function plcScaleCalcsFunc(monthstart, shift, startTime, endTime, startdate, enddate, runningtph, maxUtilization, scales, primaryScalesArray, plcIccid, flowtitle, flowiccid) {
    try {
        ...
        runtime = await runtimePlcFlow(startTime, endTime, startdate, enddate, flowtitle, flowiccid, runningtph);
