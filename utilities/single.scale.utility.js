...
async function singlecaleCalcsFunc(shift, monthstart, shift, startTime, endTime, startdate, enddate, runningtph, maxUtilization, scales, primaryScalesArray) {
    try {
        ...
        let primaryScaleIccid = 
        (scales.find(scale => scale.scaleName?.S === primaryScalesArray[0])?.iccid?.S) || 
        (scales.find(scale => scale.scaleName === primaryScalesArray[0])?.iccid || null);
