module.exports.parseFormulas = (formulas) => {
    ...
    formulas.forEach((outerFormula) => {
        ...
        jsonString=formulas[0];
         formulasArray = JSON.parse(jsonString);
...
module.exports.calculatorCalculations = async (formulas, tonsdata,flow_Values) => {
    try {
        ...
        if((flow_Values!=null) || (flow_Values!=undefined) ){
...
module.exports.calculatorCalculationsStockpile = async (formulas, tonsdata) => {
    try {


        let tonsformulas = module.exports.parseFormulas(formulas);
        var variables = tonsdata;
        ...
        console.error('Error in calculatorCalculationsStockpile:', error);
        throw new Error('Calculation failed');
    }
};
...
module.exports.getValueByKey =(array, key)=> {
    const found = array.find(item => item[key] !== undefined);
