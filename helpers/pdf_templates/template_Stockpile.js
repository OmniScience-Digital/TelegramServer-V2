...
async function PDFTableGenerator(pdfdata,sitedata, sitename, filePath, reportHeaderRenames,messageResult, options = { margin: 5 }) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(messageResult==='reset')
      {
        generatePlantName(doc, sitename + " Stockpile Management Solution",customerInformationTop);
        generatePlantName(doc,  "Premature Closure",customerInformationTop + 15,'reset');
    ...
    generateflowShifttonsGraph(doc, pdfdata.flowGraphBuffer, null, 80, 365);
    ...
    //totalizer graph
    drawRectangleWithText(doc, 'LAB QUALITY TRENDS', customerInformationTop = 520);
    ...
    await StockpileLabqualitiestable(doc, labqualities, 725, 20);


   doc.addPage();
    drawRectangleWithText(doc, 'PROGRESSIVE SHIFT  TONS', customerInformationTop = 60);
   // Draw  Shiftons table
     await Stockpiletable(doc, pdfdata.total_shifttons, 85, 20);
