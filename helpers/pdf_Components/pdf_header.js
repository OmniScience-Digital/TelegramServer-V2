...
module.exports.generatePlantName = (doc, plantName, customerInformationTop, title = 'closed') => {
  const pageWidth = 595.28;
  const textSpacer = 5;
  const lightGrayHeight = 15;
  const grayShade = '#f2f2f2';
  const textWidth = pageWidth - 2 * textSpacer - 40; // Adjust for padding

  if (title !== 'reset') {
      // Draw the light gray rectangle
      doc.fillColor(grayShade)
         .rect(20, customerInformationTop, pageWidth - 40, lightGrayHeight)
         .fill();
      
      // Draw plant name in black with underline
      doc.font('Times-Bold')
         .fontSize(9)
         .fillColor('black')
         .text(plantName.toUpperCase(), 0, customerInformationTop + textSpacer, {
             width: textWidth,
             align: 'center',
             underline: true
         })
      ...
      // Draw plant name in red for premature closure
      doc.font('Times-Bold')
         .fontSize(7)
         .fillColor('red')
         .text(plantName.toUpperCase(), 0, customerInformationTop + textSpacer, {
