...
module.exports.drawRectangleWithText = (doc, text, posY) => {
  ...
  doc.fillColor(grayShade).rect(16, posY, pageWidth - 35, lightGrayHeight).fill();

  // Draw a smaller dark gray rectangle on top
  doc.fillColor(darkGrayShade).rect(16, posY, pageWidth - 35, darkGrayHeight).fill();
...
module.exports.drawHeaderRectangles = (doc, reportHeaderRenames, reportDataArray, point) => {
  ...
  let renameHeaders = reportHeaderRenames?.L||reportHeaderRenames;
  ...
  if (renameHeaders.length < 1) {
    ...
    for (let i = 0; i < shiftstats.length; i++) {
      ...
      if (startkey.startsWith('shift') && !uniqueKeys.has(startkey)) {
        ...
        const existsShift = renameHeaders.some(header => {
          // Check if `header` is an object with an "S" key
          if (typeof header === 'object' && header.S) {
            return header.S.startsWith("Shift");
          } 
          // If `header` is a string, check directly
          else if (typeof header === 'string') {
            return header.startsWith("Shift");
          }
          return false;
        });
    ...
    for (let i = 0; i < mtdstats.length; i++) {
      ...
      if (!startkey.startsWith('shift') && !uniqueheaderKeys.has(startkey)) {
        ...
        const exists = renameHeaders.some(header => {
          // Check if `header` is an object and search within its values
          if (typeof header === 'object') {
            return Object.values(header).includes(key);
          }
          // If `header` is a string, check directly
          return header === key;
        });
...
module.exports.drawStockpileHeaderRectangles = (doc, reportHeaderRenames,sitedata, reportDataArray, point) => {
...
module.exports.Stockpiletable = async (doc, dataArray, posY, posX) => {
  ...
  //   // Draw data row for Daytons
  //   const daytonRowData = ['S / P Total', ...allKeys.map(key => (dataArray.find(entry => entry.key === key)?.Daytons || 0).toString())];
  ...
  // }
    ...
    const maxY = 790; // Adjust based on footer height
    // Calculate S/P row height
  const daytonRowData = ['S / P Total', ...allKeys.map(key => (dataArray.find(entry => entry.key === key)?.Daytons || 0).toString())];
  const daytonRowArr = daytonRowData.map((data, colIndex) => ({
    text: '   ' + data,
    width: colIndex === 0 ? 0.1 : 0.1,
  }));
  const daytonRowCellHeight = Math.max(...daytonRowArr.map(column => doc.heightOfString(column.text, { width: column.width * pageWidth }))) + textSpacer * 2;

  // Check if S/P row fits before footer
  if (y + daytonRowCellHeight > maxY) {
      doc.addPage();
      module.exports.drawRectangleWithText(doc, 'PROGRESSIVE SHIFT  TONS', customerInformationTop = 80);
      doc.font("Times-Roman").fontSize(7);
      currentPage++;
      y = 110; // Reset Y position
  }

  // Draw S/P row (now guaranteed to fit above footer)
  doc.lineWidth(0.3).strokeColor('lightgrey');
  doc.lineJoin('miter').rect(x, y, pageWidth, daytonRowCellHeight).stroke();
...
module.exports.StockpileLabqualitiestable = async (doc, dataArray, posY, posX) => {
  ...
  // Create headers
  const headers = ['Scales', ...allKeys];


  // Draw headers
  const headerArr = headers.map(header => ({
    text: '  ' + header,
    width: header === 'Scales' ? 0.115 : keyswidth, // Adjust the width as needed
  }));
...
module.exports.createSTatisticsTable = async (doc, rows, time, posY, posX, pageWidth) => {
  ...
  for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
    ...
    let textWriterPos = rows.length === 1 ? 263 : x + textSpacer;
    ...
    for (let i = 0; i < column.length; i++) {
      ...
      if (rowsLen === 1) {
        ...
        // Determine the cell width based on the column's width and page width
        const cellWidth = column[i].width * pageWidth - (textSpacer + 5);
      
        // Measure the width of the text
        const textWidth = doc.widthOfString(column[i].text);
      
        // Calculate the space on each side of the text to center it within the cell
        const remainingSpace = cellWidth - textWidth;
        const sideSpace = remainingSpace / 2;
      
        // Position text to start after side space
        const textPosition = textWriterPos + sideSpace;
      
        // Print debug information to verify equal spacing on left and right
        // console.log(`Cell Index: ${i}`);
        // console.log(`Cell Width: ${cellWidth}`);
        // console.log(`Text Width: ${textWidth}`);
        // console.log(`Remaining Space (Left + Right): ${remainingSpace}`);
        // console.log(`Side Space (Left and Right): ${sideSpace}`);
        // console.log(`Calculated Text Position: ${textPosition}`);
      
        // Draw text with calculated centered position
        doc.fillColor('black').text(column[i].text, textPosition, y + textSpacer, {
          width: cellWidth, // Width of the cell for wrapping purposes
        });
