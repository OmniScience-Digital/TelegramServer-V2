...
const scanDynamoDBTable = async () => {
  try {
    const filterExpression = 'sitestatus <> :status AND #deleted <> :deletedValue';
    const expressionAttributeValues = {
      ':status': { S: 'off' },
...
module.exports.scanDynamoDBTableWithPlcIccid = async function (plcIccid) {
  try {
    const [reports, scales] = await Promise.all([
      getDynamoDBItemByPlcIccid(plcIccid),
      getScales()
    ]);
...
const getDynamoDBItemByPlcIccid = async (plcIccid) => {
  try {
    const params = {
      TableName: reporttable,
      FilterExpression: '#plcIccid = :plcId AND sitestatus <> :status',
      ExpressionAttributeNames: {
        '#plcIccid': 'plcIccid'
      },
      ExpressionAttributeValues: {
