...
const shiftTonsAlarm = async (startTime, endTime, startdate, enddate, title, plcIccid) => {
  try {
    let query = `
      SELECT value::numeric AS shifttons
      FROM public.devicelogs_production_${plcIccid}
      WHERE 
        datasourcekey = '${title}' 
        AND status = 'OK'
        AND date BETWEEN '${startdate} ${startTime}' AND '${enddate} ${endTime}'
        AND value::numeric <> 0  -- Cast value to numeric before comparison
      ORDER BY date DESC
      LIMIT 1
    `;
...
const startingAlarmHourtons = async ( startTime, endTime, startdate, enddate, title, plcIccid) => {
  ...
  try {
    ...
    // Extract the "shifttons" value from the rows
    const starting_hourly_shifttons = result.rows;
...
exports.hourlyShifttons = async (shift, startTime, endTime, startdate, enddate, iccid, shiftFtp) => {

  try {

    let modbus_key = (shiftFtp === 'monthtons')
      ? "(datasourcekey = 'modbus-1-13')"
    ...
    if (Shifttons) {
      if (parseFloat(Shifttons[0]) < 0) {
        modbus_key = `(datasourcekey = 'modbus-1-13')`;
...
exports.hourlyStockPileShifttons = async (shift, startTime, endTime, startdate, enddate, title, plcIccid) => {
...
exports.hourlyAlarmShifttons = async ( startTime, endTime, startdate, enddate, title, plcIccid) => {

  try {


    //for period shift tons
    const Shifttons = await shiftTonsAlarm(startTime, endTime, startdate, enddate, title, plcIccid);


    //hourly Shift tons
    let myData = await startingAlarmHourtons( startTime, endTime, startdate, enddate, title, plcIccid);

    const getHourlyEnds=(myData) =>{
      const hourlyends = [];
      ...
      for (const data of myData) {
        ...
        const hour = date.getUTCHours(); // Extract the hour
      
        if (hour !== currentHour) {
          if (lastValue !== null) {
            hourlyends.push(lastValue); // Push the last value of the previous hour
          }
          currentHour = hour; // Update the current hour
        }
      
        lastValue = parseFloat(data.hourly_tons); // Update the last value
      }
      
      // Push the last value of the final hour
      if (lastValue !== null) {
        hourlyends.push(lastValue);
      }
      
      return hourlyends;
  }

 let hourlyends = getHourlyEnds(myData)
  
  return { Shifttons: Shifttons, Accumulating_Sum: parseFloat(hourlyends.length ? hourlyends[hourlyends.length - 1] : null).toFixed(2), hourlytons: hourlyends };
...
exports.getMostrecentStockpileFlow = async (modbus, iccid) => {
  try {
    const query = ` SELECT
        value FROM
        public.devicelogs_production_${iccid}
        WHERE
        datasourcekey ='${modbus}'    AND     status = 'OK'
		order by date desc
        limit 1
      `;
    ...
    // Extract the "flowdata" value from the rows
    const flowdata = result.rows[0].value;

  
    return parseFloat(flowdata);
  } catch (error) {
    console.error('Error calculating averages:', error);
    throw error;
  }
...
exports.getMostrecentStockpileTotalizer = async (modbus, iccid) => {
  try {
    const query = ` SELECT
        value FROM
        public.devicelogs_production_${iccid}
...
exports.checkStockpileTotalizerReset = async (modbus, iccid) => {
  try {
    const query = ` SELECT
        value FROM
        public.devicelogs_production_${iccid}
    ...
    // Extract the "totalizer" value from the rows
    const totalizers = result.rows;
    
    let resultantTotalizer = false;

    //if current is less than prev totalizer
    if((totalizers[0].value)<(totalizers[1].value))
    {
      //if prev is less than 7000

      if( parseFloat(totalizers[1].value) < 7000)
        resultantTotalizer=true;


    }

  
    return resultantTotalizer;
  } catch (error) {
    console.error('Error calculating averages:', error);
    throw error;
  }
...
exports.runtimeSinglePlcFlow = async (startTime, endTime, startdate, enddate, flowtitle, flowiccid, runningtph) => {
  try {
...
exports.runtimePlcFlow = async (startTime, endTime, startdate, enddate, flowtitle, flowiccid, runningtph) => {
  try {
...
//monthtodate
exports.monthToDate = async (startTime, endTime, startdate, enddate, iccid, shiftFtp) => {
  try {


    let datasource = (shiftFtp === 'monthtons')
      ? "(datasourcekey = 'modbus-1-13')"
      : "(datasourcekey = 'modbus-1-0' OR datasourcekey = 'modbus-17-0')";
...
exports.progressiveStockpile = async (title, plcIccid) => {
  try {


    const query = `SELECT value 
     FROM public.devicelogs_production_${plcIccid}
     WHERE datasourcekey = '${title}' 
     ORDER BY date DESC 
     LIMIT 1
...
exports.getStockpileValues = async (spNum, position, plcIccid) => {
  ...
  try {
    ...
    (SELECT value 
     FROM public.devicelogs_production_${plcIccid}
     WHERE datasourcekey = '${spNum}' 
     ORDER BY date DESC 
     LIMIT 1) AS "SpNum",
     
    (SELECT value 
     FROM public.devicelogs_production_${plcIccid}
     WHERE datasourcekey = '${position}' 
     ORDER BY date DESC 
     LIMIT 1) AS "position";
  ...
  `;
    ...
    console.error('Error calculating stockpile number and position:', error);
    throw error;
  }

}

exports.getProgressiveValues = async (totalizer, endTime, plcIccid) => {
...
exports.getTotalizerProgressiveValues = async (totalizer, endTime, plcIccid) => {
...
exports.getSetpointValues = async (modbus, plcIccid) => {

  try {
    const query = `
   SELECT value
  FROM public.devicelogs_production_${plcIccid}
  WHERE datasourcekey = '${modbus}'
  ORDER BY date DESC
  LIMIT 1;
  ...
  `;
    ...
    const [[setpointKey, setpointValue] = [null, null]] = Object.entries(data ?? {}) || [];
...
exports.getBatchFlowValues = async (scales, postgressStarttime, postgressEndtime) => {
  try {
    // Create a query for each ICCID and combine them with UNION ALL
    const queries = scales.map(scale => {
      return `(SELECT iccid,  value, date
              FROM public.devicelogs_production_${scale.iccid}
              WHERE date BETWEEN '${postgressStarttime}' AND '${postgressEndtime}'
              AND datasourcekey = 'modbus-1-4'
              ORDER BY date DESC)`;
    }).join(' UNION ALL ');


    // Only execute if we have scales to query
    if (queries.length === 0) {
      return [];
    }
    ...
    // Group the results by iccid for easier processing
    const groupedResults = {};
    result.rows.forEach(row => {
      if (!groupedResults[row.iccid]) {
        groupedResults[row.iccid] = [];
      }
      groupedResults[row.iccid].push(row);
    });

     return groupedResults;
  } catch (error) {
    console.error('Error fetching batch flow data:', error);
    throw error;
  }
};
